...
import TrocaSenhaPaciente from "./TrocaSenhaPaciente";
...
export const handlers = [
  rest.post(
    "http://localhost:5000/v1/seguranca/trocarSenhaPaciente",
    (req, res, ctx) => {
...
test("Renderização componente", async () => {
  expect(
    renderWithProviders(
      <Router>
        <TrocaSenhaPaciente useNavigate={() => {}} />
      </Router>
    )
  ).toMatchSnapshot();
});
...
test("Falha no preenchimento", async () => {
  ...
  //Usuário
  fireEvent.change(screen.getByPlaceholderText("Informe o email"), {
    target: { value: "teste" },
  });
  fireEvent.click(screen.getByRole("button", { name: /Trocar/i }));
